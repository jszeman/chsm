...
The project consists of two main parts: an editor (CGEN) to draw state machines and a C library (CRF) that is able to drive the C code generated by the editor.

CGEN (editor) features:
...
* The editor generates a JSON internal representation of the state machine and then uses customisable jinja templates for generating the output.
...
* Mostly follows the UML statecharts specification.
